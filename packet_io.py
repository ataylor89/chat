...
class PacketIO:
        ...
        if encryption:
            ...
            if len(encrypted_packet) == packet_len:
                if self.logging_enabled:
                    log_message = format("%s -- %s\n" %(timestamp, "Received packet (encrypted)"))
                ...
                if self.logging_enabled:
                    log_message = format("%s -- %s\n" %(timestamp, "Received packet (decrypted)"))
                    self.logger.write(log_message)
                    contents = packet_utils.hexdump(decrypted_packet)
                    self.logger.write(contents)
                    self.logger.write("\n")
                    self.logger.flush()
                    log_message = format("%s -- %s\n" %(timestamp, "Received packet (decoded)"))
                    self.logger.write(log_message)
            ...
            if self.logging_enabled:
                log_message = format("%s -- %s\n" %(timestamp, "Received packet (unencrypted)"))
                self.logger.write(log_message)
    ...
    def write_packet(self, s, packet_type, message, key=None, encryption=False):
        ...
        if encryption:
            ...
            if self.logging_enabled:
                log_message = format("%s -- %s\n" %(timestamp, "Sent packet (encrypted)"))
                self.logger.write(log_message)
                contents = packet_utils.hexdump(encrypted_packet)
                self.logger.write(contents)
                self.logger.write("\n")
                self.logger.flush()
                log_message = format("%s -- %s\n" %(timestamp, "Sent packet (decrypted)"))
                self.logger.write(log_message)
            ...
            s.sendall(packet)
            if self.logging_enabled:
                log_message = format("%s -- %s\n" %(timestamp, "Sent packet (unencrypted)"))
                self.logger.write(log_message)
                contents = packet_utils.hexdump(packet)
                self.logger.write(contents)
                self.logger.write("\n")
