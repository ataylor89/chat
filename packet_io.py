        ...
        if encryption:
            ...
            if len(encrypted_packet) == packet_len:
                ...
                if self.logging_enabled:
                    ...
                    self.logger.write("\n")
                    self.logger.flush()
                    log_message = format("%s -- %s\n" %(timestamp, "Received packet (processed/decoded)"))
                    self.logger.write(log_message)
            ...
            if self.logging_enabled:
                ...
                self.logger.write("\n")
                self.logger.flush()
                log_message = format("%s -- %s\n" %(timestamp, "Received packet (original/decoded)"))
                self.logger.write(log_message)
    ...
    def write_packet(self, s, packet_type, message, key=None, encryption=False):
        ...
        if encryption:
            ...
            if self.logging_enabled:
                ...
                self.logger.write("\n")
                self.logger.flush()
                log_message = format("%s -- %s\n" %(timestamp, "Sent packet (processed/decoded)"))
                self.logger.write(log_message)
                contents = packet_utils.str(decrypted_packet)
                self.logger.write(contents)
                self.logger.write("\n")
            ...
            if self.logging_enabled:
                ...
                self.logger.write("\n")
                self.logger.flush()
                log_message = format("%s -- %s\n" %(timestamp, "Sent packet (original/decoded)"))
                self.logger.write(log_message)
                contents = packet_utils.str(packet)
                self.logger.write(contents)
                self.logger.write("\n")
